     ...
     // DynamicJsonDocument doc(1024);
      ...
      for (JsonPair p : root) {
         ...
         if (value.is<JsonArray>()) {
            ...
            if (value[2].is<char*>()){
              char str[2]; 
              strncpy(str,value[2],1);    
              sensPortType[sensNumber] = str[0];   
            }
            else
              sensPortType[sensNumber] = 'D'; // default
            sensPortNumber[sensNumber] = value[3]; 
            printDebug("ID = ");
            printlnDebug(sensID[sensNumber]);
            printDebug("Interval = ");
            printlnDebug(sensInterval[sensNumber]);
            printDebug("Port = ");
            printDebug(sensPortType[sensNumber]);
            printlnDebug(sensPortNumber[sensNumber]);
            sensNumber++;
         }  
      ...
      for (JsonPair p : root) {
         ...
         if (value.is<JsonArray>()) {
            ...
            if (value[2].is<char*>()){
              char str[2]; 
              strncpy(str,value[2],1);    
              devPortType[devNumber] = str[0];   
            }
            else
              devPortType[devNumber] = 'D'; // default
            devPortNumber[devNumber] = value[3];   
            printDebug("ID = ");
            printlnDebug(devID[devNumber]);
            printDebug("Duration = ");
            printlnDebug(devDuration[devNumber]);
            printDebug("Port = ");
            printDebug(devPortType[devNumber]);
            printlnDebug(devPortNumber[devNumber]);
            devNumber++;
         }  
      }
